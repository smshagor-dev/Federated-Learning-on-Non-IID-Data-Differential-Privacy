# Coordinator Runtime Phase Architecture

the Coordinator Runtime phase adds a production-shaped coordinator runtime and real
cross-language communication on top of the Aggregation Core phase's C++ aggregation
...
| C++ gRPC server (`fl_coordinator_grpc_server`) | Real; built and run in Docker this phase (previously CI-only, unverified — see [docker-runtime.md](docker-runtime.md)) |
...
| Docker Compose coordinator + worker services | Real, built and run end-to-end this phase |
| Prometheus `/metrics` on the Go API | Real, added this phase (previously a broken scrape target) |
