"""Statistical primitives for repeated federated-learning experiments.

The implementation intentionally uses only the Python standard library so the
research registry can summarize results without importing the ML runtime. The
primary uncertainty interval is a deterministic percentile bootstrap CI for
the mean. Algorithm comparisons use matched-seed differences, a sign-flip
randomization test, Cohen's dz, and optional Holm correction.
"""
...
def bootstrap_mean_interval(
    ...
    *,
    ...
    rng = random.Random(_stable_seed("bootstrap-mean", seed, *normalized))
    n = len(normalized)
    bootstrap_means = []
    for _ in range(bootstrap_samples):
        sample_total = 0.0
        for _ in range(n):
            sample_total += normalized[rng.randrange(n)]
        bootstrap_means.append(sample_total / n)
...
def summarize_metric(
    ...
    *,
    ...
    """Summarize one benchmark cell without hiding seed-to-seed variance."""
    normalized = _validated_values(
        values,
        minimum_replicates=minimum_replicates,
...
def _paired_differences(
    ...
    differences = tuple(float(candidate[seed]) - float(baseline[seed]) for seed in seeds)
...
def _sign_flip_p_value(
    ...
    *,
    ...
    For up to 16 pairs the test enumerates every possible sign assignment.
    Larger studies use a deterministic Monte-Carlo sign-flip test with the
    Phipson-Smyth +1 correction, so a sampled p-value is never exactly zero.
    """
...
def compare_paired_metrics(
    ...
    *,
    ...
    """Compare algorithms on exactly matched seeds/conditions."""
    ...
    if std_difference == 0.0:
        if difference_summary.mean == 0.0:
            ...
            cohen_dz = math.copysign(math.inf, difference_summary.mean)
    else:
