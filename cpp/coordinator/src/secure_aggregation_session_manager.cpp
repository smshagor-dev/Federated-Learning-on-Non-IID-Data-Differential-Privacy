...
FixedPointEncodingProfile to_cpp_profile(const pb_coordinator::FixedPointEncodingProfile& proto_profile) {
    FixedPointEncodingProfile profile;
    profile.schema_version = proto_profile.schema_version();
    profile.rounding_rule = RoundingRule::kRoundHalfAwayFromZero;  // only one rule exists; wire field is descriptive
...
pb_coordinator::SecureAggregationAbortReason to_proto_abort_reason(SecureAggregationAbortReason reason) {
    switch (reason) {
...
SecureAggregationAbortReason to_cpp_abort_reason(pb_coordinator::SecureAggregationAbortReason reason) {
    switch (reason) {
...
std::string compute_fixed_point_profile_hash(const pb_coordinator::FixedPointEncodingProfile& profile) {
...
std::string compute_cryptographic_profile_hash(const pb_coordinator::CryptographicProviderProfile& profile) {
    std::ostringstream out;
...
std::string compute_frozen_cohort_roster_signing_bytes(const pb_coordinator::FrozenCohortRoster& roster) {
    std::ostringstream out;
...
SecureAggregationSessionManager::SecureAggregationSessionManager(SecureAggregationSessionStore* store)
    : store_(store) {}
...
void SecureAggregationSessionManager::persist_transition(const SessionRecord& record) const {
    ...
    persisted.updated_at_unix_s =
        record.state_machine.history().empty() ? record.created_at_unix_s
...
SecureAggregationSessionManager::SessionRecord& SecureAggregationSessionManager::require_session(
    ...
    if (it == sessions_.end()) {
        throw SecureAggregationSessionManagerError("unknown secure aggregation session: " + session_id);
    }
    return it->second;
}
...
pb_coordinator::SecureAggregationSessionStatus SecureAggregationSessionManager::status_of(
    ...
    for (const auto& [worker_id, contribution] : record.contributions_by_worker) {
        if (contribution.has_encoding_statistics()) {
            worst_case = std::max(worst_case, contribution.encoding_statistics().max_quantization_error());
    ...
    quantization_summary->set_average_quantization_error(sample_count > 0 ? sum / static_cast<double>(sample_count)
                                                                            : 0.0);
...
pb_coordinator::SecureAggregationSessionStatus SecureAggregationSessionManager::create_session(
    ...
    if (sessions_.count(config.session_id()) != 0) {
        throw SecureAggregationSessionManagerError("create_session: duplicate session_id: " + config.session_id());
    }
    ...
    if (config.ordered_participant_ids_size() == 0) {
        throw SecureAggregationSessionManagerError("create_session: ordered_participant_ids must not be empty");
    }
    {
        ...
        for (const auto& participant_id : config.ordered_participant_ids()) {
            ...
            if (!seen.insert(participant_id).second) {
                throw SecureAggregationSessionManagerError("create_session: duplicate participant_id: " +
                                                            participant_id);
            }
        }
    }
    ...
    if (!config.has_fixed_point_profile()) {
        throw SecureAggregationSessionManagerError("create_session: fixed_point_profile is required");
    }
    ...
    if (!bounds_proof.safe) {
        throw SecureAggregationSessionManagerError(
            "create_session: fixed_point_profile fails its domain bounds proof: " + bounds_proof.explanation);
    }
...
pb_coordinator::SecureAggregationSessionStatus SecureAggregationSessionManager::advertise_key(
    ...
    if (record.state_machine.state() != CohortState::kCohortForming &&
        ...
        throw SecureAggregationSessionManagerError(
            "advertise_key: session " + advertisement.session_id() + " is not accepting key advertisements "
            "(current state: " + to_string(record.state_machine.state()) + ")");
    }
    if (advertisement.run_id() != record.config.run_id() ||
        ...
        throw SecureAggregationSessionManagerError(
            "advertise_key: run_id/round_id/model_version does not match session " + advertisement.session_id());
    }
    const bool is_participant =
        std::find(record.config.ordered_participant_ids().begin(), record.config.ordered_participant_ids().end(),
                  advertisement.worker_id()) != record.config.ordered_participant_ids().end();
    if (!is_participant) {
        throw SecureAggregationSessionManagerError("advertise_key: worker_id " + advertisement.worker_id() +
                                                    " is not a configured participant of session " +
                                                    advertisement.session_id());
    }
    if (record.advertisements_by_worker.count(advertisement.worker_id()) != 0) {
        throw SecureAggregationSessionManagerError("advertise_key: duplicate advertisement from worker_id " +
                                                    advertisement.worker_id());
    }
    if (record.config.key_advertisement_deadline_unix_s() > 0.0 &&
        now_unix_s > record.config.key_advertisement_deadline_unix_s()) {
        throw SecureAggregationSessionManagerError("advertise_key: key advertisement deadline has passed for "
                                                    "session " +
                                                    advertisement.session_id());
    }
    ...
    try {
        ...
        throw SecureAggregationSessionManagerError(std::string("advertise_key: invalid public key encoding: ") +
                                                    error.what());
    }
    ...
    if (record.state_machine.state() == CohortState::kCohortForming) {
        record.state_machine.transition_to(CohortState::kKeyAdvertisement, now_unix_s,
                                            "first key advertisement received");
    }
...
pb_coordinator::FrozenCohortRoster SecureAggregationSessionManager::freeze_cohort(
    const std::string& session_id, double now_unix_s, const CoordinatorSigningIdentity* signing_identity) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    for (const auto& participant_id : record.config.ordered_participant_ids()) {
        if (record.advertisements_by_worker.count(participant_id) == 0) {
            throw SecureAggregationSessionManagerError(
                ...
                " has not advertised a key; the caller must abort (kDropout), never freeze a partial cohort");
        }
    }
    ...
    roster.set_fixed_point_profile_hash(compute_fixed_point_profile_hash(record.config.fixed_point_profile()));
    roster.set_cryptographic_profile_hash(compute_cryptographic_profile_hash(record.config.cryptographic_profile()));
    ...
    roster.set_cohort_commitment(
        compute_cohort_commitment(record.config.session_id(), record.config.run_id(), record.config.round_id(),
                                   record.config.model_version(), ordered_ids));
    ...
    record.state_machine.transition_to(CohortState::kCohortFrozen, now_unix_s, "cohort frozen: all participants advertised");
    record.frozen_roster = roster;
    record.frozen = true;
    record.expected_tensor_element_counts.clear();  // populated by the first accepted masked contribution
    persist_transition(record);
    return roster;
}

pb_coordinator::SecureAggregationSessionStatus SecureAggregationSessionManager::submit_masked_update(
    const pb_worker::MaskedClientUpdate& update, double now_unix_s) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    if (record.state_machine.state() != CohortState::kCohortFrozen &&
        ...
        throw SecureAggregationSessionManagerError(
            ...
            " is not accepting masked updates (current state: " + to_string(record.state_machine.state()) + ")");
    }
    if (!record.frozen) {
        throw SecureAggregationSessionManagerError("submit_masked_update: cohort is not frozen for session " +
                                                    update.session_id());
    }
    if (update.run_id() != record.config.run_id() || update.round_id() != record.config.round_id() ||
        update.model_version() != record.config.model_version()) {
        throw SecureAggregationSessionManagerError(
            "submit_masked_update: run_id/round_id/model_version does not match session " + update.session_id());
    }
    ...
    if (!update.frozen_roster_payload_hash().empty() &&
        ...
        throw SecureAggregationSessionManagerError(
            "submit_masked_update: frozen_roster_payload_hash does not match the frozen roster -- rejected "
            "(SECURE_AGGREGATION_REJECTION_REASON_COHORT_MISMATCH)");
    }
    if (!update.cryptographic_profile_hash().empty() &&
        ...
        throw SecureAggregationSessionManagerError(
            "submit_masked_update: cryptographic_profile_hash does not match the frozen roster -- rejected "
    ...
    if (!update.tensor_manifest_hash().empty() &&
        ...
        throw SecureAggregationSessionManagerError(
            "submit_masked_update: tensor_manifest_hash does not match the frozen roster -- rejected "
    ...
    if (!update.fixed_point_profile_hash().empty() &&
        ...
        throw SecureAggregationSessionManagerError(
            "submit_masked_update: fixed_point_profile_hash does not match the frozen roster -- rejected "
            "(SECURE_AGGREGATION_REJECTION_REASON_PROFILE_MISMATCH)");
    }
    if (record.advertisements_by_worker.count(update.worker_id()) == 0) {
        throw SecureAggregationSessionManagerError("submit_masked_update: worker_id " + update.worker_id() +
                                                    " is not a member of the frozen cohort for session " +
                                                    update.session_id());
    }
    if (record.contributions_by_worker.count(update.worker_id()) != 0) {
        throw SecureAggregationSessionManagerError("submit_masked_update: duplicate contribution from worker_id " +
                                                    update.worker_id());
    }
    if (record.config.masked_update_deadline_unix_s() > 0.0 &&
        now_unix_s > record.config.masked_update_deadline_unix_s()) {
        throw SecureAggregationSessionManagerError(
            "submit_masked_update: masked update deadline has passed for session " + update.session_id());
    }
    if (update.masked_tensors_size() == 0) {
        throw SecureAggregationSessionManagerError("submit_masked_update: masked_tensors must not be empty");
    }
    ...
    for (const auto& tensor : update.masked_tensors()) {
        const std::vector<std::uint64_t> values(tensor.masked_values().begin(), tensor.masked_values().end());
        const auto expected_checksum = compute_masked_values_checksum(values);
        if (expected_checksum != tensor.checksum()) {
            throw SecureAggregationSessionManagerError("submit_masked_update: checksum mismatch for tensor '" +
                                                        tensor.tensor_name() + "' from worker_id " +
                                                        update.worker_id());
        }
    }
    {
        ...
        if (compute_masked_values_checksum(weight_values) != update.masked_weight_checksum()) {
            throw SecureAggregationSessionManagerError(
                "submit_masked_update: masked_weight_checksum mismatch from worker_id " + update.worker_id());
        }
    }
    ...
    if (record.contributions_by_worker.empty()) {
        ...
        if (static_cast<std::size_t>(update.masked_tensors_size()) != record.expected_tensor_element_counts.size()) {
            throw SecureAggregationSessionManagerError(
    ...
    if (record.state_machine.state() == CohortState::kCohortFrozen) {
        record.state_machine.transition_to(CohortState::kMaskedUpdateCollection, now_unix_s,
                                            "first masked update received");
    }
...
fl::core::AggregationResult SecureAggregationSessionManager::finalize(const std::string& session_id,
                                                                       double now_unix_s,
    ...
    if (record.state_machine.state() != CohortState::kMaskedUpdateCollection) {
        throw SecureAggregationSessionManagerError("finalize: session " + session_id +
    ...
    if (record.contributions_by_worker.size() !=
        ...
        throw SecureAggregationSessionManagerError(
            "finalize: cohort is incomplete (" + std::to_string(record.contributions_by_worker.size()) + " of " +
            std::to_string(record.config.ordered_participant_ids_size()) +
            " participants contributed) -- refusing to produce a partial aggregate; the caller must abort "
            "(kDropout) instead");
    }

    record.state_machine.transition_to(CohortState::kAggregateValidation, now_unix_s,
    ...
    for (const auto& [tensor_name, expected_count] : record.expected_tensor_element_counts) {
        ...
        for (const auto& [worker_id, contribution] : record.contributions_by_worker) {
            ...
            if (!found) {
                record.state_machine.fail("finalize: tensor '" + tensor_name +
                                               "' unexpectedly missing from a contribution during aggregate "
                                               "validation (should have been caught at submission time)",
                                           now_unix_s);
                persist_transition(record);
                throw SecureAggregationSessionManagerError("finalize: internal consistency error for tensor '" +
                                                            tensor_name + "'");
            }
        }
        ...
        for (const auto ring_value : summed_ring_values) {
            decoded_values.push_back(decode_value(static_cast<std::int64_t>(ring_value), cpp_profile));
        }
    ...
    const double decoded_weight_sum = decode_value(static_cast<std::int64_t>(summed_weight_ring), cpp_profile);
    if (decoded_weight_sum <= 0.0) {
        record.state_machine.abort(SecureAggregationAbortReason::kMaskCancellationFailed, now_unix_s,
        ...
        persist_transition(record);
        throw SecureAggregationSessionManagerError(
            "finalize: decoded aggregate weight sum is not strictly positive -- aborted, not silently divided");
    }
    if (expected_weight_sum > 0.0 && std::abs(decoded_weight_sum - expected_weight_sum) > 1e-6) {
        record.state_machine.abort(
            SecureAggregationAbortReason::kMaskCancellationFailed, now_unix_s,
            "decoded aggregate weight sum (" + std::to_string(decoded_weight_sum) +
                ") does not match the expected fixed-weight sum (" + std::to_string(expected_weight_sum) +
                ") -- see finalize()'s expected_weight_sum parameter documentation");
        persist_transition(record);
        throw SecureAggregationSessionManagerError(
            "finalize: decoded aggregate weight sum does not match the expected fixed-weight total -- aborted");
    }

    for (const auto& [tensor_name, expected_count] : record.expected_tensor_element_counts) {
        (void)expected_count;
        auto& buffer = model_delta.at(tensor_name);  // non-const overload -- mutates in place, no const_cast needed
    ...
    record.state_machine.transition_to(CohortState::kCompleted, now_unix_s,
...
pb_coordinator::SecureAggregationSessionStatus SecureAggregationSessionManager::abort(
    const std::string& session_id, pb_coordinator::SecureAggregationAbortReason reason, double now_unix_s) {
    std::lock_guard<std::mutex> lock(mutex_);
    auto& record = require_session(session_id);
    if (reason == pb_coordinator::SECURE_AGGREGATION_ABORT_REASON_UNSPECIFIED) {
        throw SecureAggregationSessionManagerError("abort: a specific abort reason is required, not UNSPECIFIED");
    }
    record.state_machine.abort(to_cpp_abort_reason(reason), now_unix_s, "aborted via SecureAggregationSessionManager::abort");
    persist_transition(record);
    return status_of(record);
}
...
std::vector<pb_coordinator::SecureAggregationSessionSummary> SecureAggregationSessionManager::list() const {
    std::lock_guard<std::mutex> lock(mutex_);
...
std::optional<pb_coordinator::SecureAggregationTaskBinding> SecureAggregationSessionManager::find_binding_for_participant(
    ...
    if (state != CohortState::kCohortForming && state != CohortState::kKeyAdvertisement) {
        return std::nullopt;  // session no longer accepting advertisements -- nothing to bind a new task to
    }
    const bool is_participant =
        std::find(record.config.ordered_participant_ids().begin(), record.config.ordered_participant_ids().end(),
    ...
    binding.set_key_advertisement_deadline_unix_s(record.config.key_advertisement_deadline_unix_s());
    ...
    binding.set_secure_user_level_adjacency_model(record.config.secure_user_level_adjacency_model());
    binding.set_secure_user_level_clip_norm(record.config.secure_user_level_clip_norm());
    binding.set_secure_user_level_quantization_margin(record.config.secure_user_level_quantization_margin());
    binding.set_secure_user_level_effective_sensitivity(
        record.config.secure_user_level_effective_sensitivity());
    binding.set_secure_user_level_noise_multiplier(record.config.secure_user_level_noise_multiplier());
    ...
    binding.set_secure_user_level_sampling_assumption(record.config.secure_user_level_sampling_assumption());
    return binding;
}
...
std::optional<pb_coordinator::FrozenCohortRoster> SecureAggregationSessionManager::get_frozen_roster(
    const std::string& session_id) const {
    std::lock_guard<std::mutex> lock(mutex_);
    const auto it = sessions_.find(session_id);
...
std::vector<std::string> SecureAggregationSessionManager::sweep_expired_advertisement_deadlines(
    ...
    for (auto& [session_id, record] : sessions_) {
        const auto state = record.state_machine.state();
        if (state != CohortState::kCohortForming && state != CohortState::kKeyAdvertisement) {
            continue;  // already frozen, already terminal, or otherwise past the advertisement stage
        }
        ...
        record.state_machine.abort(SecureAggregationAbortReason::kDeadlineExceeded, now_unix_s,
    ...
    for (auto& [session_id, record] : sessions_) {
        ...
        record.state_machine.abort(SecureAggregationAbortReason::kDeadlineExceeded, now_unix_s,
                                    "masked update deadline passed with an incomplete cohort (" +
                                        ...
                                        std::to_string(record.config.cohort_size()) +
