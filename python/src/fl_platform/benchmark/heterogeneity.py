"""Realized heterogeneity metrics for federated client partitions.

Partition configuration such as a Dirichlet alpha describes how a split was
generated; it does not uniquely describe the split actually produced. These
metrics are derived from the concrete client label histograms and sample
counts so every benchmark can archive what was truly executed.
"""
...
def _kl_divergence(p: tuple[float, ...], q: tuple[float, ...]) -> float:
    total = 0.0
...
def compute_heterogeneity_vector(
    ...
    """Measure the realized label and quantity heterogeneity of a partition."""
