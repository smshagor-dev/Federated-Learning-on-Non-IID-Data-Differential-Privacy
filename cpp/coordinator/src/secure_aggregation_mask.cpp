...
bool participant_sorts_before(const std::string& a, const std::string& b) { return a.compare(b) < 0; }
...
PairwiseMaskSign resolve_pairwise_mask_sign(const std::string& self_participant_id,
    ...
    return participant_sorts_before(self_participant_id, peer_participant_id) ? PairwiseMaskSign::kAdd
                                                                                : PairwiseMaskSign::kSubtract;
}

std::uint64_t apply_pairwise_mask(std::uint64_t accumulator, std::uint64_t mask, PairwiseMaskSign sign) {
    switch (sign) {
...
std::uint64_t mask_encoded_value(std::int64_t base_encoded_value, const std::vector<SignedMask>& pairwise_masks) {
