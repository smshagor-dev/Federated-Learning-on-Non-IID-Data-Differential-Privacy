...
SigningKeyStatus signing_key_status_from_string(const std::string& value) {
    if (value == "pending") return SigningKeyStatus::kPending;
    if (value == "active") return SigningKeyStatus::kActive;
    if (value == "grace_period") return SigningKeyStatus::kGracePeriod;
    if (value == "revoked") return SigningKeyStatus::kRevoked;
    if (value == "expired") return SigningKeyStatus::kExpired;
...
SigningKeyRecord SigningKeyRegistry::register_initial_key(
    ...
    for (const auto& [other_key, other_record] : records_) {
        ...
        if (other_key.worker_id == request.worker_id &&
            effective_record(other_record, request.now_unix_s).status == SigningKeyStatus::kActive) {
            throw SigningKeyRegistryError(
...
SigningKeyRotationResult SigningKeyRegistry::validate_rotation(
    ...
    if (current_it == records_.end()) {
        return {false, SigningKeyRotationRejectionReason::kUnknownCurrentKey,
               "unknown current signing key '" + request.current_signing_key_id + "' for worker_id '" +
                   request.worker_id + "'"};
    }
    ...
    if (effective_current.status != SigningKeyStatus::kActive) {
        return {false, SigningKeyRotationRejectionReason::kCurrentKeyNotActive,
    ...
    if (!is_valid_ed25519_public_key_hex(request.new_public_key_hex)) {
        return {false, SigningKeyRotationRejectionReason::kInvalidKeyLength,
               "new_public_key_hex is not a valid Ed25519 public key encoding"};
    }

    for (const auto& [other_key, other_record] : records_) {
        if (other_key.signing_key_id == request.new_signing_key_id) {
            return {false, SigningKeyRotationRejectionReason::kDuplicateNewKeyId,
        ...
        if (other_record.public_key_fingerprint == request.new_public_key_fingerprint) {
            return {false, SigningKeyRotationRejectionReason::kDuplicatePublicKey,
    ...
    if (request.grace_period_seconds < 0.0 || request.grace_period_seconds > kMaxGracePeriodSeconds) {
        return {false, SigningKeyRotationRejectionReason::kExcessiveGracePeriod,
    ...
    if (request.new_key_expires_at_unix_s != 0.0 &&
        ...
        return {false, SigningKeyRotationRejectionReason::kInvalidExpiry,
    ...
    for (const auto& [other_key, other_record] : records_) {
        if (other_key.worker_id == request.worker_id &&
            ...
            effective_record(other_record, request.now_unix_s).status == SigningKeyStatus::kActive) {
            return {false, SigningKeyRotationRejectionReason::kMaxActiveKeysExceeded,
                   "worker_id '" + request.worker_id +
...
SigningKeyRecord SigningKeyRegistry::revoke_key(const std::string& worker_id,
    ...
    if (it == records_.end()) {
        throw SigningKeyRegistryError("unknown signing key '" + signing_key_id + "' for worker_id '" +
                                      worker_id + "'");
    }
...
std::optional<SigningKeyRecord> SigningKeyRegistry::find_active(const std::string& worker_id,
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    for (const auto& [key, record] : records_) {
        if (key.worker_id != worker_id) continue;
...
std::vector<std::pair<std::string, std::string>> SigningKeyRegistry::sweep_expired(
    ...
    for (auto& [key, record] : records_) {
        const auto effective = effective_record(record, now_unix_s);
        if (effective.status == SigningKeyStatus::kExpired && record.status != SigningKeyStatus::kExpired) {
