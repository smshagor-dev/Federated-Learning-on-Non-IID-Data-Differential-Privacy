...
def build_per_fedavg_step(
    ...
    if not (len(weights) == len(support_gradients) == len(query_gradients)):
        raise ValueError("weight and gradient vectors must align")

    adapted_weights = [
        weight - config.inner_lr * support_gradient
        for weight, support_gradient in zip(weights, support_gradients, strict=True)
    ]
    gradient_source = query_gradients if config.first_order else support_gradients
    meta_weights = [
        weight - config.meta_lr * gradient
        for weight, gradient in zip(adapted_weights, gradient_source, strict=True)
    ]
