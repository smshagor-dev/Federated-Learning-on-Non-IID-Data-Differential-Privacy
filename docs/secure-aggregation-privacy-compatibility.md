# Secure Aggregation x Privacy Mode Compatibility Matrix
...
covers algorithm x privacy-mode compatibility, unrelated to secure
aggregation) - this document covers privacy mode x secure aggregation
...
| `USER_LEVEL_DP` | **Yes, including secure adaptive clipping** | Algorithm must be `fedavg`; weighting must be `uniform` (fixed weight 1 per user); the run's `user_level_privacy` config (`initial_clipping_bound`, `noise_multiplier`, `target_delta`) must all be finite and positive (`target_delta` additionally `< 1`); the effective sensitivity (`clip_norm + quantization_margin`) must stay strictly below the fixed-point profile's `max_input_magnitude`; the projected epsilon after one more accountant step must stay below `epsilon_budget` (when a budget is configured); when adaptive clipping is enabled its secure adaptive-clipping configuration must also pass the bound/quantile/learning-rate/noise validation ladder |
| `HYBRID_DP` | **Yes, including secure adaptive clipping** | Every `USER_LEVEL_DP` condition (weighting/clip-norm/noise/target-delta/quantization-margin/budget/adaptive-clipping validation) applies to the user-level layer, **plus** the run's `sample_level_privacy` config (`noise_multiplier`, `max_grad_norm`, `target_delta`) must all be finite and positive (`target_delta` additionally `< 1`); sample-level and user-level budgets/accountants are validated and reported completely separately - see `docs/secure-hybrid-dp-semantics.md` |
...
| Adaptive clipping enabled outside `USER_LEVEL_DP`/`HYBRID_DP` | `SECURE_ADAPTIVE_CLIPPING_UNSUPPORTED_PRIVACY_MODE` |
...
Adaptive-clipping-specific rejections (checked only when adaptive
clipping is enabled for a secure `USER_LEVEL_DP`/`HYBRID_DP` run):

| Condition | Rejection reason |
|---|---|
| Current/min/max bound, target quantile, learning rate, or indicator noise multiplier invalid | `SECURE_ADAPTIVE_CLIPPING_INVALID_CONFIGURATION` |
| Signed task / worker submission carries a stale or mismatched adaptive configuration hash or clip-state step count | `SECURE_ADAPTIVE_CLIPPING_STALE_CONFIGURATION` |
...
See that slice's own report,
[threshold-recovery-dependency-decision.md](threshold-recovery-dependency-decision.md),
and `docs/secure-user-level-dp-semantics.md`'s Mandatory Privacy Trust
Statement.
