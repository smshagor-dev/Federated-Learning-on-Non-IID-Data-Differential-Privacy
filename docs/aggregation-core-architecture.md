...
## Status: implemented and tested (the Aggregation Core phase scope: FedAvg, FedProx,
...
are the Coordinator Runtime phase scaffolding and are explicitly out of scope for this
report.
...
cpp/core/include/fl_core/
    ...
    coordinator.hpp     RunState machine, ClientScheduler, CoordinatorCheckpoint (Coordinator Runtime phase scaffold)
...
* No coordinator round lifecycle, client scheduling, or gRPC service — those
  ...
  in `coordinator.hpp` as the Coordinator Runtime phase scaffolding, compiled and smoke-tested
...
* No LibTorch dependency — tensor math is plain `std::vector<double>`
  ...
  of a heavy external dependency for the Aggregation Core phase's scope.
