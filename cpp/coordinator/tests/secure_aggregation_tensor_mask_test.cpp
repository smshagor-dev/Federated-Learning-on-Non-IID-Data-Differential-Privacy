...
int main() {
    ...
    {
        const std::string fixed_secret = hex_decode(
            "000102030405060708090a0b0c0d0e0f"
            "101112131415161718191a1b1c1d1e1f");
        const auto golden_stream = derive_tensor_mask_stream(
            fixed_secret, kHkdfPurposeTensorMaskStream, "golden-fixture-context", 4);
        check(golden_stream.size() == 4,
              "golden fixture: derive_tensor_mask_stream produces 4 elements");
        const std::vector<std::uint64_t> expected{9086732726997875728ULL,
                                                  16617846648716935654ULL,
                                                  15486973003086668688ULL,
                                                  10999587187094550710ULL};
    ...
    // -- mask stream derivation: uniqueness and key sensitivity -----------
    {
        ...
        const auto stream_a =
            derive_tensor_mask_stream(secret_a, kHkdfPurposeTensorMaskStream, "ctx", 8);
        const auto stream_b =
            derive_tensor_mask_stream(secret_b, kHkdfPurposeTensorMaskStream, "ctx", 8);
        check(stream_a.size() == 8,
              "derive_tensor_mask_stream produces the requested element count");
        check(stream_a != stream_b,
              "a different shared secret derives a completely different mask stream");

        const auto stream_a_again =
            derive_tensor_mask_stream(secret_a, kHkdfPurposeTensorMaskStream, "ctx", 8);
        check(stream_a == stream_a_again,
              "derive_tensor_mask_stream is deterministic for identical inputs");

        const auto stream_a_different_context =
            derive_tensor_mask_stream(secret_a, kHkdfPurposeTensorMaskStream, "different-ctx", 8);
        check(stream_a != stream_a_different_context,
              "a different canonical context derives a different mask stream");
    ...
    // -- mask_tensor combining --------------------------------------------
    {
        ...
        check(masked[0] == static_cast<std::uint64_t>(100 + 10 - 1),
              "mask_tensor combines masks element-wise (element 0)");
        check(masked[1] == static_cast<std::uint64_t>(-50 + 20 - 2),
              "mask_tensor combines masks element-wise (element 1)");
        check(masked[2] == static_cast<std::uint64_t>(0 + 30 - 3),
              "mask_tensor combines masks element-wise (element 2)");

        std::vector<PeerMaskStream> mismatched_peers;
        mismatched_peers.push_back(
            PeerMaskStream{"peer-a", PairwiseMaskSign::kAdd, {10, 20}});  // wrong length
        expect_throw(
            [&]() { (void)mask_tensor(encoded_tensor, mismatched_peers); },
            "mask_tensor rejects a peer mask stream whose length does not match the tensor");
    }

    // -- sum_masked_tensors --------------------------------------------
    {
        const std::vector<std::vector<std::uint64_t>> tensors{
            {1, 2, 3}, {10, 20, 30}, {100, 200, 300}};
        const auto sum = sum_masked_tensors(tensors);
        check(sum == std::vector<std::uint64_t>{111, 222, 333},
              "sum_masked_tensors sums element-wise across participants");

        expect_throw([]() { (void)sum_masked_tensors({}); },
                     "sum_masked_tensors rejects an empty participant list");
        expect_throw([&]() { (void)sum_masked_tensors({{1, 2}, {1, 2, 3}}); },
    ...
    // -- weight masking (Work Package U) -----------------------------
    {
        ...
        check(
            weight_mask1 == tensor_mask_same_inputs.front(),
    ...
    // ======================================================================
    {
        ...
        const std::vector<double> true_values{
            1.5, -2.25, 3.75, -1.0};  // one scalar contribution per participant
        ...
        for (std::size_t i = 0; i < participants.size(); ++i) {
            for (std::size_t j = i + 1; j < participants.size(); ++j) {
                ...
                const auto secret_reverse =
                    derive_x25519_shared_secret(keypairs[participants[j]].private_key_raw,
                                                keypairs[participants[i]].public_key_raw);
                check(secret == secret_reverse,
                      "both sides of every pairwise X25519 exchange in the cohort agree");
                shared_secrets[{participants[i], participants[j]}] = secret;
            }
        }
        auto lookup_shared_secret = [&](const std::string& a,
        ...
        for (const auto& self_id : participants) {
            const auto self_index = static_cast<std::size_t>(
                std::find(participants.begin(), participants.end(), self_id) -
                participants.begin());
            ...
            for (const auto& peer_id : participants) {
                ...
                const std::string context =
                    session_id + "|" + std::to_string(round_id) + "|scalar|" +
                    (participant_sorts_before(self_id, peer_id) ? self_id + "|" + peer_id
                                                                : peer_id + "|" + self_id);
                const auto mask_value =
                    derive_weight_mask(secret, kHkdfPurposeWeightMaskStream, context);
        ...
        const double decoded_complete =
            decode_value(static_cast<std::int64_t>(complete_sum), profile);
        double true_sum = 0.0;
        for (const double v : true_values) true_sum += v;
        check(std::abs(decoded_complete - true_sum) < 1e-6,
              "CAPSTONE: a complete, honest 4-participant cohort's real masked-sum decodes to the "
              "exact true "
        ...
        const std::vector<std::uint64_t> partial{
            masked_contributions[0], masked_contributions[1], masked_contributions[2]};
        const auto partial_sum = sum_masked_values(partial);
        const double decoded_partial =
            decode_value(static_cast<std::int64_t>(partial_sum), profile);
        ...
        check(std::abs(decoded_partial - true_partial_sum) > 1e-3,
              "CAPSTONE: an incomplete cohort's masked-sum does NOT recover the true partial "
              "aggregate -- "
              "pairwise masks against the missing participant do not cancel, concretely "
              "demonstrating why a "
              "post-freeze dropout must abort the session rather than silently degrade to a "
              "partial aggregate");
    }

    if (g_failures == 0) {
        std::cout << "all secure aggregation tensor mask tests passed (including the capstone "
                     "cancellation proof)\n";
    }
