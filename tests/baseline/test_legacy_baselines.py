...
class LegacyBaselineTests(unittest.TestCase):
    ...
    def test_dirichlet_partitioning_is_seeded(self) -> None:
        part_a = partition_dirichlet(self.dataset, num_clients=4, alpha=0.3, seed=5, min_partition_size=2)
        part_b = partition_dirichlet(self.dataset, num_clients=4, alpha=0.3, seed=5, min_partition_size=2)
        for key in part_a:
            self.assertTrue(np.array_equal(part_a[key], part_b[key]))

    def test_pathological_partitioning_is_seeded(self) -> None:
        part_a = partition_pathological(self.dataset, num_clients=4, classes_per_client=1, seed=9)
        part_b = partition_pathological(self.dataset, num_clients=4, classes_per_client=1, seed=9)
        for key in part_a:
            self.assertTrue(np.array_equal(part_a[key], part_b[key]))

    def test_rdp_accountant_progresses(self) -> None:
        accountant = MomentsAccountant(noise_multiplier=0.8, sample_rate=0.25, target_delta=1e-5)
    ...
    def test_weight_variance_and_client_drift_are_deterministic(self) -> None:
        ...
        self.assertAlmostEqual(compute_client_drift([delta_a, delta_b]), np.sqrt(0.5), places=6)

    def test_server_fedavg_weighting(self) -> None:
        model = build_model("cnn", num_classes=4, in_channels=1, group_norm_groups=2)
        server = Server(model=model, num_clients=4, algorithm="fedavg", server_lr=1.0, device=torch.device("cpu"))
        before = copy.deepcopy(server.broadcast())
        float_key = next(name for name, tensor in before.items() if torch.is_floating_point(tensor))
    ...
    def test_client_dp_disabled_preserves_local_delta(self) -> None:
        ...
        result = client.train(model=model, global_state=global_state, algorithm="fedavg")
        recomputed = {
