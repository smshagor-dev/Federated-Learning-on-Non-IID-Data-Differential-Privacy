...
fl::coordinator::ClientResultSubmission make_submission(const std::string& client_id,
                                                        double value) {
...
void run_task_dispatcher_tests() {
...

    {
        ...
        check(dispatcher.pending_count() == 1,
              "acquiring a task removes it from the pending queue");

        const auto second_attempt = dispatcher.acquire("worker-1", 0.0);
        check(!second_attempt.has_value(),
              "a worker already holding a lease cannot acquire a second task");
...

    {
        ...
        const auto duplicate = dispatcher.submit_result("worker-1",
                                                        task.task_id,
                                                        task.lease_id,
                                                        make_submission("c1", 2.0),
                                                        2.0,
                                                        duplicate_reason);
...

    {
        ...
        std::string reason;
        const auto accepted = dispatcher.submit_result("worker-1",
                                                       task.task_id,
                                                       task.lease_id,
                                                       make_submission("c1", 1.0),
                                                       /*now=*/11.0,
                                                       reason);
...

    {
        ...
        auto failed = dispatcher.sweep_expired_leases(
            6.0);  // expired: attempt 1 < max_retries(2) -> requeued
        ...
        failed = dispatcher.sweep_expired_leases(
            12.0);  // attempt 2 >= max_retries(2) -> permanently failed
        check(failed.size() == 1 && failed[0] == "c1",
              "exhausting max_retries permanently fails the task");
