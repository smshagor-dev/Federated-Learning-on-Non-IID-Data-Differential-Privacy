...
void run_cohort_state_machine_tests() {
    ...
    {
        check(to_string(SecureAggregationProvider::kNone) == "NONE", "SecureAggregationProvider::kNone stringifies as NONE");
        check(to_string(SecureAggregationProvider::kSecureAggregationNoDropoutExperimental) ==
                  "SECAGG_NO_DROPOUT_EXPERIMENTAL",
              "the real provider's name communicates the no-dropout/experimental limitation directly");
    }
    ...
    // A fresh state machine starts in CohortForming, non-terminal, no history.
    {
        ...
        check(machine.state() == CohortState::kCohortForming, "a fresh state machine starts in COHORT_FORMING");
    ...
    // The full happy-path forward sequence, one step at a time.
    {
        ...
        check(machine.state() == CohortState::kKeyAdvertisement, "COHORT_FORMING -> KEY_ADVERTISEMENT succeeds");

        machine.transition_to(CohortState::kCohortFrozen, 2.0, "cohort frozen");
        check(machine.state() == CohortState::kCohortFrozen, "KEY_ADVERTISEMENT -> COHORT_FROZEN succeeds");

        machine.transition_to(CohortState::kMaskedUpdateCollection, 3.0, "collecting masked updates");
        ...
        check(machine.state() == CohortState::kCompleted, "AGGREGATE_VALIDATION -> COMPLETED succeeds");
    ...
    // Skipping a state is rejected -- no implicit shortcuts.
    {
        ...
        expect_throw([&]() { machine.transition_to(CohortState::kCohortFrozen, 1.0); },
    ...
    // it never silently continues with a partial cohort.
    {
        ...
        forming.abort(SecureAggregationAbortReason::kManualAbort, 1.0, "operator cancelled before freeze");
        ...
        frozen.abort(SecureAggregationAbortReason::kDropout, 4.0, "worker-3 did not submit before deadline");
        check(frozen.state() == CohortState::kAborted,
              "MASKED_UPDATE_COLLECTION aborts on a post-freeze dropout, never proceeds with a partial cohort");
    ...
    // and marks the machine terminal with a recorded reason.
    {
        ...
        check(machine.state() == CohortState::kFailed, "fail() moves the machine to FAILED regardless of prior state");
        check(machine.is_terminal(), "FAILED is a terminal state");
        check(machine.failure_reason() == "unexpected internal error: manifest hash mismatch mid-round",
    ...
    {
        check(to_string(CohortState::kCohortForming) == "COHORT_FORMING", "CohortState::kCohortForming stringifies as expected");
        check(to_string(CohortState::kAborted) == "ABORTED", "CohortState::kAborted stringifies as expected");
        check(to_string(CohortState::kFailed) == "FAILED", "CohortState::kFailed stringifies as expected");
