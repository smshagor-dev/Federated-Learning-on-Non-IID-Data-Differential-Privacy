...
SecureAggregationCryptoError::SecureAggregationCryptoError(const std::string& what) : std::runtime_error(what) {}
...
X25519KeyPair generate_x25519_keypair() {
    ...
    EVP_PKEY_free(pkey);
    if (!got_private || !got_public || priv_len != kX25519KeyLength || pub_len != kX25519KeyLength) {
        throw SecureAggregationCryptoError("failed to extract raw key material from the generated X25519 keypair");
    }
...
std::string derive_x25519_shared_secret(const std::string& self_private_key_raw,
    ...
    if (self_private_key_raw.size() != kX25519KeyLength) {
        throw SecureAggregationCryptoError("derive_x25519_shared_secret: self private key is not 32 bytes");
    }
    if (peer_public_key_raw.size() != kX25519KeyLength) {
        throw SecureAggregationCryptoError("derive_x25519_shared_secret: peer public key is not 32 bytes");
    }

    EVP_PKEY* self_pkey = EVP_PKEY_new_raw_private_key(
        EVP_PKEY_X25519, nullptr, reinterpret_cast<const unsigned char*>(self_private_key_raw.data()),
        self_private_key_raw.size());
    if (self_pkey == nullptr) {
        throw SecureAggregationCryptoError("derive_x25519_shared_secret: failed to load self private key");
    }
    EVP_PKEY* peer_pkey = EVP_PKEY_new_raw_public_key(
        EVP_PKEY_X25519, nullptr, reinterpret_cast<const unsigned char*>(peer_public_key_raw.data()),
        peer_public_key_raw.size());
    if (peer_pkey == nullptr) {
        EVP_PKEY_free(self_pkey);
        throw SecureAggregationCryptoError("derive_x25519_shared_secret: failed to load peer public key");
    }

    EVP_PKEY_CTX* dctx = EVP_PKEY_CTX_new(self_pkey, nullptr);
    std::string secret;
    bool ok = false;
    if (dctx != nullptr && EVP_PKEY_derive_init(dctx) == 1 && EVP_PKEY_derive_set_peer(dctx, peer_pkey) == 1) {
    ...
    if (dctx != nullptr) EVP_PKEY_CTX_free(dctx);
    EVP_PKEY_free(peer_pkey);
    EVP_PKEY_free(self_pkey);

    if (!ok) {
        throw SecureAggregationCryptoError("derive_x25519_shared_secret: X25519 shared-secret derivation failed");
    }
    ...
    // zero."
    const bool all_zero = std::all_of(secret.begin(), secret.end(), [](char byte) { return byte == '\0'; });
    if (all_zero) {
...
std::string hkdf_sha256(const std::string& salt, const std::string& ikm, const std::string& info,
    ...
    ok = ok && EVP_PKEY_CTX_set1_hkdf_salt(kctx, reinterpret_cast<const unsigned char*>(salt.data()),
                                            static_cast<int>(salt.size())) == 1;
    ok = ok && EVP_PKEY_CTX_set1_hkdf_key(kctx, reinterpret_cast<const unsigned char*>(ikm.data()),
                                           static_cast<int>(ikm.size())) == 1;
    ok = ok && EVP_PKEY_CTX_add1_hkdf_info(kctx, reinterpret_cast<const unsigned char*>(info.data()),
    ...
    if (ok) {
        ...
        ok = EVP_PKEY_derive(kctx, reinterpret_cast<unsigned char*>(out.data()), &actual_len) == 1 &&
...
std::string derive_purpose_key(const std::string& shared_secret, const std::string& purpose_label,
                                const std::string& canonical_context, std::size_t output_length) {
...
std::string chacha20_keystream(const std::string& key, const std::string& nonce, std::uint32_t initial_counter,
                                std::size_t length) {
...
std::string sha256_hex(const std::string& data) { return hex_encode(sha256_digest(data)); }

std::string compute_cohort_commitment(const std::string& session_id, const std::string& run_id,
...
std::string compute_session_configuration_hash(const SecureAggregationSessionConfig& config) {
    ...
    // field values.
    std::ostringstream out;
    out.write(kSessionConfigHashPrefix, static_cast<std::streamsize>(kSessionConfigHashPrefixLength));
    ...
    out << "key_advertisement_deadline_unix_s=" << config.key_advertisement_deadline_unix_s << '\x1e';
