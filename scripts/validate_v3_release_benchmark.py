...
def main() -> int:
    ...
    _require(
        plan.get("benchmark_id") == "v3-release-baseline",
        "benchmark id mismatch",
    )
    _require(
        plan.get("datasets") == ["MNIST"],
        "release benchmark must use MNIST only",
    )
    _require(
        plan.get("algorithms") == ["fedavg"],
        "release benchmark must use FedAvg",
    )
    _require(
        plan.get("target_epsilons") == [None],
        "release benchmark must be non-private",
    )
    _require(
        plan.get("runtime_identity") == "root-simulator",
        "runtime identity mismatch",
    )
    _require(
        plan.get("rounds") == 1,
        "release benchmark must run one qualification round",
    )
    _require(
        tuple(plan.get("seeds", ())) == EXPECTED_SEEDS,
        "release seed set mismatch",
    )
    ...
    _require(
        partitions[0].get("name") == "iid",
        "release partition must be iid",
    )
    _require(
        partitions[0].get("strategy") == "iid",
        "release partition strategy mismatch",
    )

    _require(isinstance(statuses, list), "benchmark status must be a list")
    _require(
        len(statuses) == len(EXPECTED_SEEDS),
        "expected exactly five benchmark cells",
    )
    _require(
    ...
    _require(
        ...
        "benchmark has no observations",
    )
    _require(
        isinstance(summaries, list) and summaries,
        "benchmark has no summaries",
    )

    metrics_by_seed: dict[int, set[str]] = {seed: set() for seed in EXPECTED_SEEDS}
    for observation in observations:
        _require(
            observation.get("benchmark_id") == "v3-release-baseline",
            "observation id mismatch",
        )
        _require(
            observation.get("dataset_id") == "MNIST",
            "observation dataset mismatch",
        )
        _require(
            observation.get("algorithm_id") == "fedavg",
            "observation algorithm mismatch",
        )
        _require(
            observation.get("partition_id") == "iid",
            "observation partition mismatch",
        )
        _require(
            observation.get("target_epsilon") is None,
            "observation must be non-private",
        )
        _require(
            observation.get("target_delta") is None,
            "non-private delta must be null",
        )
        _require(
            observation.get("runtime_identity") == "root-simulator",
            "observation runtime mismatch",
        )
        _require(
            str(observation.get("commit_sha", "")).lower() == commit_sha,
            "observation commit mismatch",
        )
