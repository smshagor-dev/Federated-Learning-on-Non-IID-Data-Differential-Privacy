...
2. The client delta is clipped to norm $C = \texttt{dp.update\_clip\_norm}$ when DP is enabled.
...
$$
\widetilde{\Delta}_t = \sum_{k \in S_t}\operatorname{clip}(\Delta_k, C) + \mathcal{N}(0, \sigma^2 C^2 I),
$$
